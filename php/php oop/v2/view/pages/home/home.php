...
class Person implements iPerson, InfoInterface {
  public $name;
  private $age;
...
interface VihicleInterface {
  public function setColor($color);
  public function showVihicle();
...
Class Car extends CarBrand implements VihicleInterface, InfoInterface {
  private $color;
  public $carName;
  public $fuel;
  ...
  public function getInfo() {
    return [$this->type, $this->color, $this->fuel, $this->carName, $this->brandName];
  }

}

require_once('model/dbhandler.php');
$database = new DbHandler('localhost', 'car', 'root', '');
$cars = $database->readData(['selectQuery' => 'SELECT car.car_name, car.color, fuel.fuel_name, brand.brand_name FROM car LEFT JOIN brand ON car.brand_brand_id = brand.brand_id LEFT JOIN fuel ON car.fuel_fuel_id = fuel.fuel_id WHERE car.brand_brand_id = 1']);
...
foreach($array as $row) {
  echo $row->showVihicle() . '<br>' . 'Sustainability: ' . $row->checkSustainability() .  '<br><br>';
}
?>
