...
class HtmlGenerator {
	...
	public function createUpdateForm($array, $sendUrl, $addition = '') {
		$string = '<section class="updateForm">';
		...
		$string .= $addition;
		$string .= '<a id="updateForm" href="javascript:;">Save</a>';	
		$string .= '</section>';
		return $string;
	}


	//this method requires google fonts!!!!!@!@!@!@
	public function createUserSchema($array, $editUrl) {
		$string = '<section class="user">';
		$counter = 0;
		foreach($array as $row) {
			if($counter < 1) {
				$string .= '<div>';
			...
			$string .= '<p><a href="' . $editUrl . array_values($row)[0] . '"><i class="material-icons">edit</i></a></p>';
			$string .= '</div>';
